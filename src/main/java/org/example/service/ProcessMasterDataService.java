package org.example.service;

import org.example.entity.ProcessMasterDataItem;
import org.example.repository.ProcessMasterDataItemRepository;
import org.springframework.stereotype.Service;

import java.util.List;
import java.util.Optional;

@Service
public class ProcessMasterDataService {

    public static final String ZM_OBSERVATION = "ZM_OBSERVATION";
    public static final String PM_OBSERVATION = "PM_OBSERVATION";
    public static final String QM_OBSERVATION = "QM_OBSERVATION";

    private final ProcessMasterDataItemRepository repository;

    public ProcessMasterDataService(ProcessMasterDataItemRepository repository) {
        this.repository = repository;
    }

    public List<ProcessMasterDataItem> list(String category) {
        return repository.findByCategoryOrderByNameAsc(normalizeCategory(category));
    }

    public ProcessMasterDataItem add(String category, String name) {
        String normalizedCategory = normalizeCategory(category);
        String normalizedName = normalizeName(name);
        rejectDuplicate(normalizedCategory, normalizedName, null);

        ProcessMasterDataItem item = new ProcessMasterDataItem();
        item.setCategory(normalizedCategory);
        item.setName(normalizedName);
        return repository.save(item);
    }

    public Optional<ProcessMasterDataItem> update(Long id, String name) {
        if (id == null) {
            return Optional.empty();
        }
        Optional<ProcessMasterDataItem> existing = repository.findById(id);
        if (existing.isEmpty()) {
            return Optional.empty();
        }

        ProcessMasterDataItem item = existing.get();
        String normalizedName = normalizeName(name);
        rejectDuplicate(item.getCategory(), normalizedName, id);
        item.setName(normalizedName);
        return Optional.of(repository.save(item));
    }

    public boolean delete(Long id) {
        if (id == null || !repository.existsById(id)) {
            return false;
        }
        repository.deleteById(id);
        return true;
    }

    private String normalizeCategory(String category) {
        String normalized = category == null ? "" : category.trim().toUpperCase().replace('-', '_');
        if (ZM_OBSERVATION.equals(normalized) || PM_OBSERVATION.equals(normalized) || QM_OBSERVATION.equals(normalized)) {
            return normalized;
        }
        throw new IllegalArgumentException("Unsupported category");
    }

    private String normalizeName(String name) {
        if (name == null || name.trim().isEmpty()) {
            throw new IllegalArgumentException("Name is required");
        }
        return name.trim();
    }

    private void rejectDuplicate(String category, String name, Long allowedId) {
        Optional<ProcessMasterDataItem> duplicate = repository.findByCategoryAndNameIgnoreCase(category, name);
        if (duplicate.isPresent() && (allowedId == null || !duplicate.get().getId().equals(allowedId))) {
            throw new IllegalArgumentException("Name already exists");
        }
    }
}
