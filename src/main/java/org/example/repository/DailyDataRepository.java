...
public interface DailyDataRepository extends JpaRepository<DailyData, Long> {
    ...
    @Query("SELECT MAX(d.date) FROM DailyData d WHERE d.type = :type AND d.date <= :endDate")
    LocalDate findLatestDateByTypeOnOrBefore(@Param("type") String type, @Param("endDate") LocalDate endDate);
